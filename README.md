...
* **Baseline Modes:** Baseline defaults to **No baseline**. **Single control request - may mutate state** sends one control request and shares that fingerprint only with queued requests that have the same method, URL, body, and request headers after excluding generated transport headers such as `Content-Length`. Unsafe methods such as POST require confirmation. **Full baseline (destructive)** sends every queued request sequentially before racing and requires confirmation because it can consume one-time application state.
...
* **Queue Table:** A clear table showing every queued request, its attempt, method, URL, status, normalized UTF-8 byte length, timing, response family hash, attempt order, batch order, and anomaly notes. Before arming, staged requests can be removed, duplicated, moved up/down, or cleared.
* **Real-Time Feedback:** The UI reports active attempt readiness before release, then updates each row with attempt number, status code, normalized UTF-8 byte length, timing, thread dispatch offset, body hash, attempt-local response order, batch-wide response order, and anomaly summary.
...
* Ignored JSON path normalization is reliable for top-level fields, but nested paths are currently normalized by final field name only. Ignoring `$.user.id` can also redact another `"id"` field such as `$.order.id`. A proper JSON parser would make nested extraction and normalization more precise.
...
* Full response bodies are intentionally not retained for every row. A row can still be analyzed through status, normalized length, body hash, attempt order, batch order, dispatch timing, selected headers, and anomaly text.
